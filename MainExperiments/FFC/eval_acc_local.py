...
focus_method = {'test_method':300,'energy':300}
...
def get_change_rate(conf, bs):
    ...
    for i in range(1,step_num):
        ...
        remain_acc = conf_step.argmax(-1)
        rate[i] = (remain_acc==pred_label).sum()
...
def print_info(msg, file_name = 'eval_acc_local.log'):
    if log_en:
        f = open(file_name,'a')
        print(msg, file=f)
        f.close()
    ...
    for batch,(X,y) in enumerate(val_loader):
        ...
        if method_name in ['fullgrad','inputgrad','smoothgrad','gradcam','smooth_grad','input_grad']:
            ...
            with torch.enable_grad():
                scores = test_method(model_name, X)
        else:
        ...
        if batch % 10 == 0:
            ...
            print_info(msg)
    sum_acc = rate.sum().item()
    rate /= 50000
    rate = rate.cpu()
    rate = rate.numpy().tolist()
    msg = method_name+":"+model_name+":"+str(rate)+"\nSum Acc:"+str(sum_acc)
    print_info(msg,'temp_acc_eval_res.log')
    return rate
